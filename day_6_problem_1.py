...
be any number of objects long: if A orbits B, B orbits C, and C orbits D,
    ...
    if m in map:
        if m not in done:
            ...
            for i in range(len(map[m])):
                total += count_all_orbits(map[m][i], map, done, depth + 1)
        else:
            total += len(map[m])
    return total + depth
...
def start():
    ...
    map = map_direct_orbits(orbit_map)

    for m in map:
        if m not in done:
            total += count_all_orbits(m, map, done, 0)
