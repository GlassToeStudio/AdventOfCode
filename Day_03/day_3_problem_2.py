...
class Coords:
    def __init__(self, x, y, steps, id):
        self.x = x
        self.y = y
        self.steps = steps
        self.id = id
...
def map_wire_path(wire_path, coord_dict, intersections_dict, id):
    ...
    j = 0
    steps = 0
    length = len(wire_path)

    for i in range(length):
        direction = wire_path[i][0]
        distance = int(wire_path[i][1:])

        if direction == RIGHT:
            for i in range(distance):
                c = Coords(x + i, y, steps, id)
                h = c.hash_coord()
                if h in coord_dict and coord_dict[h].id != id:
                    intersections_dict[j] = coord_dict[h], c
                    j += 1
                else:
                    coord_dict[h] = c
                steps += 1
    ...
    return coord_dict, intersections_dict


def find_fewest_steps(coord_dict, intersections_dict):
    ''' Return the fewest steps to an intersection. '''

    closest = math.inf
    for intersection in intersections_dict.values():
        if (intersection[0].x == 0 and intersection[0].y == 0 and
                intersection[1].x == 0 and intersection[1].y == 0):
            pass
        else:
            c = intersection[0].steps + intersection[1].steps
            if c < closest:
                closest = c
...
if __name__ == "__main__":
    ...
    coord_dict, intersections_dict = map_wire_path(
        wire_path_1,
        coord_dict,
        intersections_dict,
        1)
    ...
    fewest_steps = find_fewest_steps(coord_dict, intersections_dict)
    print(f"The fewest combined steps to an intersection is {fewest_steps}")
